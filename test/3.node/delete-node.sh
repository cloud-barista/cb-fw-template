...
# usage
if [ "$#" -lt 1 ]; then 
	echo "delete-node.sh [GCP/AWS] <clsuter name> <node name>"
	echo "    ./delete-node.sh GCP cb-cluster cb-gcp-cluster-test-0-w-asdf"
	exit 0; 
fi
...
delete() {
	...
	curl -sX DELETE http://$MyfwServer/myfw/ns/$NS_ID/clusters/$CLUSTER_ID/nodes/${v_NODE_NAME}    -H "${c_CT}" | jq;
